...
const PaginationLink = props => {
  if (!props.test) {
    return <Link to={props.url} className="button is-rounded">{props.text}</Link>;
  } else {
    return <Link disabled={true} className="button is-rounded" to={props.url}>{props.text}</Link>;
  }
};
